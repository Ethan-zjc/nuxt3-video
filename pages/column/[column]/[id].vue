        ...
        <div class="items-center">
            ...
            <el-breadcrumb separator-class="el-icon-arrow-right">
                ...
                <el-breadcrumb-item v-if="detailRes">
                    {{
                        detailRes.data.title.length > 12
                        ? detailRes.data.title.substr(0, 12) + '...'
        ...
        <el-row :gutter="40" class="mt-20">
            <el-col :span="18" :xs="24">
                <h1 class="movie-detail-title hidden-sm-and-up">
                    {{ detailRes?.data.title }}
                    <span :class="detailRes?.data.movieRate?.rateUserCount > 0
                        ? 'rate'
                        : ''
                        ">
                        {{
                            detailRes?.data.movieRate?.rateUserCount > 0
                            ? detailRes?.data.movieRate.rate.toFixed(1)
                            : '暂无评分'
                        }}
                    </span>
                </h1>
                    ...
                    <div class="movies-info">
                        <h1 class="hidden-sm-and-down">
                            ...
                            <span :class="detailRes?.data.movieRate?.rateUserCount > 0
                                ...
                                {{
                                    detailRes?.data.movieRate?.rateUserCount > 0
                                    ? detailRes?.data.movieRate.rate.toFixed(
                                        1
                                    )
                                    : '暂无评分'
                                }}
                        ...
                        <el-form label-position="right">
                            ...
                            <div>
                                ...
                                <el-button :loading="collectLoading" :icon="isCollect ? ElIconStarFilled : ElIconStar"
                                    @click="handleCollect">
                                    {{ isCollect ? '已收藏' : '收藏' }}
                                </el-button>
                                <ClientOnly>
                ...
                <div class="mt-20" v-if="detailRes?.data.movieVideos &&
                    detailRes.data.movieVideos.length
                    ">
                        ...
                        <ul>
                            <li v-for="item in castsRes.rows">
                                <el-image style="width: 110px; height: 156px" fit="cover"
                                    :src="item.actorAvatar"></el-image>
                                <div>{{ item.actorName }}</div>
                                <div class="grey" v-if="item.role">
                                    饰 {{ item.role }}
                                </div>
...
const [{ data: detailRes, refresh }, { data: castsRes }] = await Promise.all([
    ...
    useServerRequest<ResPage<CastItem[]>>(
        `movie/cast/list?movieId=${id}&pageNum=1&pageSize=50`
    ),
]);
...
// 获取榜单
const { data: rank } = await useServerRequest<MovieLeaderboard>(
    'movie/leaderboard',
    {
        query: {
