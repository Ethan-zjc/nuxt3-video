    ...
    <div class="container mt-20 show">
        ...
        <el-row :gutter="40">
            <el-col :span="18" :xs="24">
                ...
                <el-form>
                    <el-form-item label="按类型">
                        <ul class="show__type-filter">
                            ...
                            <li v-for="(item, index) in genreList.data" :class="route.query.t === item.name ? 'active' : ''"
                                :key="index">
                                <nuxt-link :to="{ path: route.path, query: { ...route.query, t: item.name } }">{{ item.name
                                }}</nuxt-link>
                            </li>
                        </ul>
                    </el-form-item>
                    <el-form-item label="按地区">
                        <ul class="show__type-filter">
                            ...
                            <li v-for="(item, index) in countryList.data"
                                :class="+route.query.c === +item.id ? 'active' : ''">
                                <nuxt-link :to="{ path: route.path, query: { ...route.query, c: item.id } }">{{ item.name
                                }}</nuxt-link>
                            </li>
                        </ul>
                    </el-form-item>
                    <el-form-item label="按年份">
                        <ul class="show__type-filter">
                            ...
                            <li v-for="(item, index) in yearList" :class="+route.query.y === item ? 'active' : ''">
                                <nuxt-link :to="{ path: route.path, query: { ...route.query, y: item } }">{{ item
                                }}</nuxt-link>
                            </li>
                        </ul>
                    </el-form-item>
                    <el-form-item label="按语言">
                        <ul class="show__type-filter">
                            ...
                            <li v-for="(item, index) in languageList.data"
                                ...
                                <nuxt-link :to="{ path: route.path, query: { ...route.query, l: item.name } }">{{ item.name
                                }}</nuxt-link>
                            </li>
                        </ul>
                    </el-form-item>
                ...
                <div class="video-list" v-loading="pending">
                    ...
                    <div class="pagination" v-if="movieList.total !== 0">
                        <el-pagination background layout="prev, pager, next" :current-page="currentPage" :page-size="30"
                            :pager-count="5" :total="movieList.total" @current-change="handleCurrentChange" />
                    </div>
            ...
            <el-col :span="6" class="hidden-sm-and-down">
                <!--   周榜单     -->
                <Ranking title="周榜单" :list="rank.data.weekRank"></Ranking>
                
                <!--   月榜单     -->
                <Ranking title="月榜单" :list="rank.data.mouthRank"></Ranking>
            </el-col>
        </el-row>
    </div>
</template>
