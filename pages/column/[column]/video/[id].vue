                            ...
                            <li v-for="item in detail.videos">
                                <nuxt-link :to="`/column/${detail.movie.columnValue}/video/${item.id}`">
                                    <el-image class="img" fit="cover"
                                        :src="item.cover || item.video?.poster || ''"></el-image>
                                    <p :title="item.title"><span
                                            :class="+item.id === +route.params.id ? 'animate' : ''">{{+item.id ===
                                                +route.params.id ? '正在播放: ' : '' }}{{ item.title }}</span></p>
                                </nuxt-link>
                            </li>
                        </ul>
                    </div>
...
onMounted(async () => {
    ...
    player = new Player.default({
        ...
        autoplay: true,
        volume: 0.3,
        url: process.env.NODE_ENV === 'development' ? detail.videoInfo?.url.replace('https://cms.yinchunyu.com/server', '/server') : detail.videoInfo?.url,
        playsinline: true,
        height: '100%',
        width: '100%',
        ...
        payTip: {
            ...
            arriveTime() {
                // 用户没有购买 && 影片设置了需要购买 && 正片
                if (!isUserBuy.value && +detail.movie.isPay === 1 && +detail.typeId === 1) {
                    player?.pause()
...
onMounted(async () => {
    ...
    player = new Player.default({
        ...
        autoplay: true,
        volume: 0.3,
        url: 'http://api.yinchunyu.com/upload/videos/1692672213029-658837048-%C3%A5%C2%AD%C2%A4%C3%A6%C2%B3%C2%A8%C3%A4%C2%B8%C2%80%C3%A6%C2%8E%C2%B7.mp4',
        playsinline: true,
        height: '100%',
        width: '100%',
